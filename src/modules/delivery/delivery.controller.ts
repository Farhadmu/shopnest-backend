...
import {
  ...
  getDeliveryMyReverseRequests as getDeliveryMyReverseRequestsService,
  getDeliveryReverseRequests as getDeliveryReverseRequestsService,
  updateReverseDeliveryStatus as updateReverseDeliveryStatusService,
...
export const createDeliveryIncident = asyncHandler(async (req: Request, res: Response) => {
  const { deliveryRequestId, reverseDeliveryRequestId, returnRequestId, orderId, productId, category, severity, description, evidenceImages } = req.body as {
    deliveryRequestId?: string;
  ...
  let linkedReverse: any = null;
  const targetDelId = reverseDeliveryRequestId || deliveryRequestId || orderId;
  ...
  if (targetDelId && targetDelId !== "general") {
    ...
    if (linkedDelivery) {
      ...
      linkedReverse = await ReverseDeliveryRequest.findOne({
        $or: [
          ...(isObjId ? [{ _id: new mongoose.Types.ObjectId(targetDelId) }] : []),
          { orderId: targetDelId },
        ],
      ...
      if (linkedReverse) {
        if (linkedReverse.assignedDeliveryManId !== userId && !isAdmin) {
          throw ApiError.forbidden("You can only report incidents for your assigned deliveries");
        }
      }
    }
  ...
  const incident = await DeliveryIncident.create({
    ...
    reverseDeliveryRequestId: linkedReverse?.id || reverseDeliveryRequestId || undefined,
    returnRequestId: linkedReverse?.returnRequestId || returnRequestId || undefined,
    orderId: linkedDelivery?.orderId || linkedReverse?.orderId || orderId || undefined,
...
export const getAvailableReverseDeliveries = asyncHandler(async (req: Request, res: Response) => {
  const requests = await getDeliveryReverseRequestsService(req.user!.id);
  sendSuccess(res, requests);
});

export const getMyReverseDeliveries = asyncHandler(async (req: Request, res: Response) => {
  const requests = await getDeliveryMyReverseRequestsService(req.user!.id);
  sendSuccess(res, requests);
});

export const acceptReverseDelivery = asyncHandler(async (req: Request, res: Response) => {
  const claimed = await acceptReverseDeliveryService(req.params.id, req.user!.id, req.user!.name || "Delivery Partner");
  sendSuccess(res, { reverseDelivery: claimed });
});
...
export const updateReverseDeliveryStatus = asyncHandler(async (req: Request, res: Response) => {
  ...
  const reverse = await updateReverseDeliveryStatusService(
    req.params.id,
    status,
    req.user!.id,
    req.user!.role,
    { failureReason, deliveryOtp, deliveryProofImage, note }
  );
  sendSuccess(res, { reverseDelivery: reverse });
});
